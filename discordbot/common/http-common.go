...
import (
	...
	"net/http/httputil"
	"net/url"
	"raccoon-mh-playground/models"
	"strings"
)

// CommonCaller
func CommonCaller(callMethod string, targetUrl string, endPoint string, commonRequest *models.CommonRequest, authheader string, auth string) (*models.CommonResponse, error) {
	...
	commonResponse, err := CommonHttpToCommonResponse(requestUrl, commonRequest.Request, callMethod, authheader, auth)
...
func CommonHttpToCommonResponse(url string, s interface{}, httpMethod string, authheader string, auth string) (*models.CommonResponse, error) {
